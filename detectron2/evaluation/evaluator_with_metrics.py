...
def get_multi_r2(true, pred, fold=0, step="default_0"):
    ...
    with open(f"/home/ll610/Onepiece/code_cs138/github/detectron2/experimental_record_fold_{fold}.txt", "a") as text_file:
        text_file.write(f"fold_{fold}_step_{step} r2_list: " + str(r2_list).split("multi_r2")[-1] + "\n")
...
def inference_on_dataset(
    model, data_loader, evaluator: Union[DatasetEvaluator, List[DatasetEvaluator], None], fold=0, step='0'
):
    """
    ...
    with ExitStack() as stack:
        ...
        print(f"Fold {fold}, Step {step}:")
        with open(f"/home/ll610/Onepiece/code_cs138/github/detectron2/experimental_record_fold_{fold}.txt", "a") as text_file:
            text_file.write(f"Fold {fold}, Step {step}:" + "\n")

        all_counts_np = np.asarray(all_counts)

        df = pd.DataFrame(data=all_counts_np, columns=["neutrophil", "epithelial", "lymphocyte", "plasma", "eosinophil", "connective"])
        df.to_csv(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/counts_pred_fold_{fold}_step_{step}.csv', index=False)
        ...
        pred_csv = pd.read_csv(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/counts_pred_fold_{fold}_step_{step}.csv')
        true_csv = pd.read_csv(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/valid_true_fold_{fold}.csv')
        all_metrics = {}
        reg_metrics_names = ["r2"]
        for idx, metric in enumerate(reg_metrics_names):
            if metric == "r2":
                # calculate multiclass coefficient of determination
                r2 = get_multi_r2(true_csv, pred_csv, fold=fold, step=step)
        ...
        print("r2_avg: "+str(df).split("\n")[-1])
        with open(f"/home/ll610/Onepiece/code_cs138/github/detectron2/experimental_record_fold_{fold}.txt", "a") as text_file:
            text_file.write(f"fold_{fold}_step_{step} r2_avg: " + str(df).split("\n")[-1] + "\n")


        if segment_flag:
            np.save(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/valid_pred_fold_{fold}_step_{step}.npy', segmentation_results)
            ...
            pred_array = np.load(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/valid_pred_fold_{fold}_step_{step}.npy')
            true_array = np.load(f'/home/ll610/Onepiece/code_cs138/github/mmdetection/conic/dataset/conic/detectron2_results/fold_{fold}/valid_true_fold_{fold}.npy')
            ...
            for idx, metric in enumerate(seg_metrics_names):
                if metric == "multi_pq+":
                    ...
                    print("mqp_list: " + str(mpq_list).split("\n")[-1])
                    with open(f"/home/ll610/Onepiece/code_cs138/github/detectron2/experimental_record_fold_{fold}.txt", "a") as text_file:
                        text_file.write(f"fold_{fold}_step_{step} mqp_list: " + str(mpq_list).split("\n")[-1] + "\n")
            ...
            df = pd.DataFrame(all_metrics)
            df = df.to_string(index=False)
            print("mpg_avg: " + str(df).split("\n")[-1])
            with open(f"/home/ll610/Onepiece/code_cs138/github/detectron2/experimental_record_fold_{fold}.txt", "a") as text_file:
                text_file.write(f"fold_{fold}_step_{step} mpg_avg: " + str(df).split("\n")[-1] + "\n\n\n")
